...
/// Generate a new FEM miner key: "FEM-" + 32 UPPERCASE hex chars (16 random bytes).
/// Uppercase matches the fleet-wide key convention (dashboard generateMinerKey,
/// FryHub-migrated keys); lowercase generation produced server-side device docs
/// the dashboard's case-sensitive claim lookup could never match (D1).
...
/// Validate a FEM key WITHOUT re-casing it: trim, require FEM- prefix
/// (case-insensitive), exactly 32 alphanumeric characters, return "FEM-" +
/// the body exactly as provided. The server treats miner keys as opaque
/// case-sensitive identities — displaying or submitting a re-cased key
/// desyncs the device from its server-side records (D1: unclaimable devices,
/// duplicate docs from token migration).
pub fn validate_fem_key_preserve_case(input: &str) -> Result<String, String> {
...
/// DEPRECATED for lookups/display: uppercasing desyncs from case-sensitive
/// server-side keys — use validate_fem_key_preserve_case instead.
#[allow(dead_code)] // retained for API parity; superseded by validate_fem_key_preserve_case
...
mod tests {
    ...
    #[test]
    fn test_generate_uppercase_body() {
        // Lowercase generated keys produced server docs the dashboard's
        // case-sensitive claim lookup could never match (D1).
        let key = generate();
        let body = key.strip_prefix("FEM-").unwrap();
        assert_eq!(body, body.to_uppercase());
    }

    #[test]
    fn test_validate_preserve_case_keeps_lowercase_body() {
        let key = "fem-abcdef0123456789abcdef0123456789";
        let out = validate_fem_key_preserve_case(key).unwrap();
        assert_eq!(out, "FEM-abcdef0123456789abcdef0123456789");
    }

    #[test]
    fn test_validate_preserve_case_keeps_uppercase_body() {
    ...
    #[test]
    fn test_validate_preserve_case_rejects_bad_input() {
        assert!(validate_fem_key_preserve_case("FEM-short").is_err());
        assert!(validate_fem_key_preserve_case("BM-abcdef0123456789abcdef0123456789").is_err());
        assert!(validate_fem_key_preserve_case("FEM-____________0123456789abcdef____").is_err());
    }
